...
describe("IssueDetail (shared)", () => {
  beforeEach(() => {
    ...
    mockApiObj.getServer.mockResolvedValue({
      id: "server-1",
      workspace_id: "ws-1",
      name: "prod-node-01",
      host: "10.0.0.12",
    ...
    mockApiObj.getServerMetrics.mockResolvedValue({
      cpu_idle_percent: 74.5,
      memory_free_percent: 61.2,
      disk_free_percent: 48.1,
      gpu_free_percent: 32.4,
      collected_at: "2026-01-20T12:00:00Z",
    });
  ...
  it("renders server context in the sidebar when ops context is attached", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Server context")).toBeInTheDocument();
    });
    ...
    expect(screen.getByText("74.5% CPU idle")).toBeInTheDocument();
  });
});
