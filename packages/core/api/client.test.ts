...
describe("ApiClient", () => {
  ...
  it("uses the expected HTTP contract for server endpoints", async () => {
    const fetchMock = vi.fn().mockImplementation(() => Promise.resolve(
      new Response(JSON.stringify({ servers: [], total: 0, id: "srv-1" }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }),
    ));
    ...
    await client.updateServer("srv-1", {
      environment: "staging",
      tags: ["staging"],
    });
    await client.getServerMetrics("srv-1");
    await client.refreshServerMetrics("srv-1");
    ...
    expect(calls).toMatchObject([
      { url: "https://api.example.test/api/servers", method: "GET" },
      { url: "https://api.example.test/api/servers/srv-1", method: "GET" },
      {
        url: "https://api.example.test/api/servers",
        method: "POST",
        body: JSON.stringify({
          name: "prod-node-01",
          host: "10.0.0.12",
      ...
      {
        url: "https://api.example.test/api/servers/srv-1",
        method: "PATCH",
        body: JSON.stringify({
          environment: "staging",
          tags: ["staging"],
      ...
      { url: "https://api.example.test/api/servers/srv-1/metrics", method: "GET" },
      { url: "https://api.example.test/api/servers/srv-1/metrics/refresh", method: "POST" },
      { url: "https://api.example.test/api/servers/srv-1", method: "DELETE" },
    ]);
  });
});
