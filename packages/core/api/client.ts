import type {
  ...
  ListServerRelatedIssuesResponse,
  PinnedItem,
  CreatePinRequest,
  PinnedItemType,
  ReorderPinsRequest,
...
export class ApiClient {
  ...
  async deleteServer(id: string): Promise<void> {
    await this.fetch(`/api/servers/${id}`, { method: "DELETE" });
  }

  async getServerMetrics(id: string): Promise<ServerMetricSnapshot> {
    return this.fetch(`/api/servers/${id}/metrics`);
  }

  async listServerRelatedIssues(id: string): Promise<ListServerRelatedIssuesResponse> {
    return this.fetch(`/api/servers/${id}/issues`);
  }

  async refreshServerMetrics(id: string): Promise<ServerMetricSnapshot> {
    return this.fetch(`/api/servers/${id}/metrics/refresh`, {
      method: "POST",
    });
  }
