...
type IssueResponse struct {
	...
	OpsContext         *IssueOpsContextResponse `json:"ops_context,omitempty"`
	Reactions          []IssueReactionResponse `json:"reactions,omitempty"`
	Attachments        []AttachmentResponse    `json:"attachments,omitempty"`
}

type IssueOpsContextResponse struct {
...
func issueOpsContextToResponse(ctx db.IssueOpsContext) IssueOpsContextResponse {
	return IssueOpsContextResponse{
		TargetServerID: uuidToPtr(ctx.TargetServerID),
		OpsIntent:      textToPtr(ctx.OpsIntent),
		RiskLevel:      textToPtr(ctx.RiskLevel),
		ExecutionMode:  textToPtr(ctx.ExecutionMode),
	}
}

func optionalTrimmedText(value *string) pgtype.Text {
	if value == nil {
		return pgtype.Text{}
	}
...
func hasIssueOpsContext(req CreateIssueRequest) bool {
	return optionalTrimmedString(req.TargetServerID) != nil ||
		optionalTrimmedString(req.OpsIntent) != nil ||
		optionalTrimmedString(req.RiskLevel) != nil ||
		optionalTrimmedString(req.ExecutionMode) != nil
}
...
func (h *Handler) GetIssue(w http.ResponseWriter, r *http.Request) {
	...
	if opsContext, err := h.Queries.GetIssueOpsContextByIssueInWorkspace(r.Context(), db.GetIssueOpsContextByIssueInWorkspaceParams{
		IssueID:     issue.ID,
		WorkspaceID: issue.WorkspaceID,
	}); err == nil {
		mapped := issueOpsContextToResponse(opsContext)
		resp.OpsContext = &mapped
	} else if !isNotFound(err) {
		slog.Warn("get issue ops context failed", append(logger.RequestAttrs(r), "error", err, "issue_id", uuidToString(issue.ID))...)
	}
...
func (h *Handler) CreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	normalizedTargetServerID := optionalTrimmedString(req.TargetServerID)
	var targetServerID pgtype.UUID
	if normalizedTargetServerID != nil {
		server, err := h.Queries.GetServerInWorkspace(r.Context(), db.GetServerInWorkspaceParams{
			ID:          parseUUID(*normalizedTargetServerID),
			WorkspaceID: workspaceUUID,
		})
		if err != nil {
			writeError(w, http.StatusBadRequest, "target server not found in this workspace")
			return
		}
	...
	var opsContextResp *IssueOpsContextResponse
	if hasIssueOpsContext(req) {
		opsContext, err := qtx.UpsertIssueOpsContext(r.Context(), db.UpsertIssueOpsContextParams{
			ID:             issue.ID,
			ID_2:           targetServerID,
			OpsIntent:      optionalTrimmedText(req.OpsIntent),
			RiskLevel:      optionalTrimmedText(req.RiskLevel),
			ExecutionMode:  optionalTrimmedText(req.ExecutionMode),
			WorkspaceID:    workspaceUUID,
		})
		if err != nil {
			writeError(w, http.StatusInternalServerError, "failed to create issue ops context")
			return
		}
		mapped := issueOpsContextToResponse(opsContext)
		opsContextResp = &mapped
	}
	...
	prefix := h.getIssuePrefix(r.Context(), issue.WorkspaceID)
	resp := issueToResponse(issue, prefix)
	resp.OpsContext = opsContextResp
