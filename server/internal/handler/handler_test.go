...
func TestIssueCRUDWithOpsContext(t *testing.T) {
	var serverID, issueID string
	defer func() {
		if issueID != "" {
			req := newRequest("DELETE", "/api/issues/"+issueID, nil)
			req = withURLParam(req, "id", issueID)
		...
		if serverID != "" {
			req := newRequest("DELETE", "/api/servers/"+serverID, nil)
			req = withURLParam(req, "id", serverID)
			testHandler.DeleteServer(httptest.NewRecorder(), req)
		}
	}()

	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/servers?workspace_id="+testWorkspaceID, map[string]any{
		"name":           "issue-target-node",
		"host":           "10.0.0.21",
		"environment":    "prod",
		"ssh_username":   "root",
		"credential_ref": "cred:test:issue-target-node",
	})
	...
	var createdServer ServerResponse
	if err := json.NewDecoder(w.Body).Decode(&createdServer); err != nil {
		t.Fatalf("CreateServer: failed to decode response: %v", err)
	}
	serverID = createdServer.ID

	w = httptest.NewRecorder()
	req = newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":            "Drain prod node before maintenance",
		"status":           "todo",
		"priority":         "high",
	...
	var created struct {
		...
		OpsContext *struct {
			...
			OpsIntent      *string `json:"ops_intent"`
	...
	issueID = created.ID
	if created.OpsContext == nil {
		t.Fatal("CreateIssue: expected ops_context in response")
	}
	if created.OpsContext.TargetServerID == nil || *created.OpsContext.TargetServerID != serverID {
		t.Fatalf("CreateIssue: expected target_server_id %q, got %v", serverID, created.OpsContext.TargetServerID)
	}
	if created.OpsContext.OpsIntent == nil || *created.OpsContext.OpsIntent != "drain node" {
		t.Fatalf("CreateIssue: expected ops_intent to round-trip, got %v", created.OpsContext.OpsIntent)
	}
	if created.OpsContext.RiskLevel == nil || *created.OpsContext.RiskLevel != "high" {
		t.Fatalf("CreateIssue: expected risk_level to round-trip, got %v", created.OpsContext.RiskLevel)
	}
	if created.OpsContext.ExecutionMode == nil || *created.OpsContext.ExecutionMode != "guided" {
		t.Fatalf("CreateIssue: expected execution_mode to round-trip, got %v", created.OpsContext.ExecutionMode)
	}
	...
	if fetched.OpsContext == nil {
		t.Fatal("GetIssue: expected ops_context in response")
	}
	if fetched.OpsContext.TargetServerID == nil || *fetched.OpsContext.TargetServerID != serverID {
		t.Fatalf("GetIssue: expected target_server_id %q, got %v", serverID, fetched.OpsContext.TargetServerID)
	}
}
...
func TestServerCRUD(t *testing.T) {
	...
	if created.Name != "prod-node-01" {
		t.Fatalf("CreateServer: expected name 'prod-node-01', got %q", created.Name)
	}
	if created.Host != "10.0.0.12" {
		t.Fatalf("CreateServer: expected host '10.0.0.12', got %q", created.Host)
	}
	if created.CredentialRef != "cred:test:prod-node-01" {
		t.Fatalf("CreateServer: expected credential_ref to round-trip, got %q", created.CredentialRef)
	}
	if len(created.Tags) != 2 {
		t.Fatalf("CreateServer: expected 2 tags, got %d", len(created.Tags))
	}
	serverID = created.ID

	w = httptest.NewRecorder()
	req = newRequest("GET", "/api/servers", nil)
	testHandler.ListServers(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("ListServers: expected 200, got %d: %s", w.Code, w.Body.String())
	}
	...
	if err := json.NewDecoder(w.Body).Decode(&listResp); err != nil {
		t.Fatalf("ListServers: failed to decode response: %v", err)
	}
	if len(listResp.Servers) == 0 {
		t.Fatal("ListServers: expected at least 1 server")
	}

	w = httptest.NewRecorder()
	req = newRequest("GET", "/api/servers/"+serverID, nil)
	req = withURLParam(req, "id", serverID)
	testHandler.GetServer(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("GetServer: expected 200, got %d: %s", w.Code, w.Body.String())
	}

	var fetched ServerResponse
	if err := json.NewDecoder(w.Body).Decode(&fetched); err != nil {
		t.Fatalf("GetServer: failed to decode response: %v", err)
	}
	if fetched.ID != serverID {
		t.Fatalf("GetServer: expected id %q, got %q", serverID, fetched.ID)
	}

	w = httptest.NewRecorder()
	req = newRequest("PATCH", "/api/servers/"+serverID, map[string]any{
		"name":        "prod-node-01-renamed",
		"environment": "staging",
	})
	req = withURLParam(req, "id", serverID)
	testHandler.UpdateServer(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("UpdateServer: expected 200, got %d: %s", w.Code, w.Body.String())
	}

	var updated ServerResponse
	if err := json.NewDecoder(w.Body).Decode(&updated); err != nil {
		t.Fatalf("UpdateServer: failed to decode response: %v", err)
	}
	if updated.Name != "prod-node-01-renamed" {
		t.Fatalf("UpdateServer: expected updated name, got %q", updated.Name)
	}
	if updated.Environment != "staging" {
		t.Fatalf("UpdateServer: expected updated environment, got %q", updated.Environment)
	}
	if updated.Host != "10.0.0.12" {
		t.Fatalf("UpdateServer: host should be preserved, got %q", updated.Host)
	}

	w = httptest.NewRecorder()
	req = newRequest("DELETE", "/api/servers/"+serverID, nil)
	req = withURLParam(req, "id", serverID)
	testHandler.DeleteServer(w, req)
	if w.Code != http.StatusNoContent {
		t.Fatalf("DeleteServer: expected 204, got %d: %s", w.Code, w.Body.String())
	}
	serverID = ""

	w = httptest.NewRecorder()
	req = newRequest("GET", "/api/servers/"+created.ID, nil)
	req = withURLParam(req, "id", created.ID)
	testHandler.GetServer(w, req)
	if w.Code != http.StatusNotFound {
		t.Fatalf("GetServer after delete: expected 404, got %d: %s", w.Code, w.Body.String())
	}
}
...
func TestGetServerMetrics(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/servers?workspace_id="+testWorkspaceID, map[string]any{
		"name":           "metrics-node-01",
		"host":           "10.0.0.88",
		"environment":    "prod",
		"ssh_username":   "ubuntu",
		"credential_ref": "cred:test:metrics-node-01",
	})
	testHandler.CreateServer(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateServer: expected 201, got %d: %s", w.Code, w.Body.String())
	...
	assertRange := func(name string, got, min, max float64) {
		if got < min || got > max {
			t.Fatalf("%s: expected %.2f to be within [%.2f, %.2f]", name, got, min, max)
		}
	}

	w = httptest.NewRecorder()
	req = newRequest("GET", "/api/servers/"+serverID+"/metrics", nil)
	req = withURLParam(req, "id", serverID)
	testHandler.GetServerMetrics(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("GetServerMetrics: expected 200, got %d: %s", w.Code, w.Body.String())
	}

	var initial metricsResponse
	if err := json.NewDecoder(w.Body).Decode(&initial); err != nil {
		t.Fatalf("GetServerMetrics: failed to decode response: %v", err)
	}
	assertRange("cpu_idle_percent", initial.CPUIdlePercent, 35, 90)
	assertRange("memory_free_percent", initial.MemoryFreePercent, 20, 85)
	assertRange("disk_free_percent", initial.DiskFreePercent, 25, 92)
	assertRange("gpu_free_percent", initial.GPUFreePercent, 0, 100)
	if initial.CollectedAt == "" {
		t.Fatal("GetServerMetrics: expected collected_at")
	}

	w = httptest.NewRecorder()
	req = newRequest("POST", "/api/servers/"+serverID+"/metrics/refresh", nil)
	req = withURLParam(req, "id", serverID)
	testHandler.RefreshServerMetrics(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("RefreshServerMetrics: expected 200, got %d: %s", w.Code, w.Body.String())
	}

	var refreshed metricsResponse
	if err := json.NewDecoder(w.Body).Decode(&refreshed); err != nil {
		t.Fatalf("RefreshServerMetrics: failed to decode response: %v", err)
	}
	assertRange("refreshed cpu_idle_percent", refreshed.CPUIdlePercent, 35, 90)
	assertRange("refreshed memory_free_percent", refreshed.MemoryFreePercent, 20, 85)
	assertRange("refreshed disk_free_percent", refreshed.DiskFreePercent, 25, 92)
	assertRange("refreshed gpu_free_percent", refreshed.GPUFreePercent, 0, 100)
	if refreshed.CollectedAt == initial.CollectedAt &&
		refreshed.CPUIdlePercent == initial.CPUIdlePercent &&
		refreshed.MemoryFreePercent == initial.MemoryFreePercent &&
		refreshed.DiskFreePercent == initial.DiskFreePercent &&
		refreshed.GPUFreePercent == initial.GPUFreePercent {
		t.Fatal("RefreshServerMetrics: expected refresh to produce a newer or different snapshot")
	}
}

func TestGetServerRelatedIssues(t *testing.T) {
	var serverID, issueID string
	defer func() {
		if issueID != "" {
			req := newRequest("DELETE", "/api/issues/"+issueID, nil)
	...
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/servers?workspace_id="+testWorkspaceID, map[string]any{
		"name":           "related-node-01",
		"host":           "10.0.0.101",
		"environment":    "prod",
		"ssh_username":   "ubuntu",
		"credential_ref": "cred:test:related-node-01",
	})
	testHandler.CreateServer(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateServer: expected 201, got %d: %s", w.Code, w.Body.String())
	...
	w = httptest.NewRecorder()
	req = newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":            "Patch kernel on related node",
		"description":      "Roll the latest kernel update during maintenance window.",
		"target_server_id": serverID,
		"ops_intent":       "patch",
		"risk_level":       "medium",
	...
	if err := json.NewDecoder(w.Body).Decode(&createdIssue); err != nil {
		t.Fatalf("CreateIssue: failed to decode response: %v", err)
	}
	issueID = createdIssue.ID

	w = httptest.NewRecorder()
	req = newRequest("GET", "/api/servers/"+serverID+"/issues", nil)
	req = withURLParam(req, "id", serverID)
	testHandler.GetServerRelatedIssues(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("GetServerRelatedIssues: expected 200, got %d: %s", w.Code, w.Body.String())
	}
	...
	if err := json.NewDecoder(w.Body).Decode(&related); err != nil {
		t.Fatalf("GetServerRelatedIssues: failed to decode response: %v", err)
	}
	if len(related.Issues) != 1 {
		t.Fatalf("GetServerRelatedIssues: expected 1 issue, got %d", len(related.Issues))
	}
	if related.Issues[0].ID != issueID {
		t.Fatalf("GetServerRelatedIssues: expected issue id %q, got %q", issueID, related.Issues[0].ID)
	}
	if related.Issues[0].Title != "Patch kernel on related node" {
		t.Fatalf("GetServerRelatedIssues: expected title to round-trip, got %q", related.Issues[0].Title)
	}
	if related.Issues[0].Identifier != createdIssue.Identifier {
		t.Fatalf("GetServerRelatedIssues: expected identifier %q, got %q", createdIssue.Identifier, related.Issues[0].Identifier)
	}
	if related.Issues[0].Status != createdIssue.Status {
		t.Fatalf("GetServerRelatedIssues: expected status %q, got %q", createdIssue.Status, related.Issues[0].Status)
	}
}

func TestCreateIssueRejectsCrossWorkspaceServerBinding(t *testing.T) {
	const otherWorkspaceSlug = "handler-ops-other-workspace"
	ctx := context.Background()
	var otherWorkspaceID, otherServerID string

	t.Cleanup(func() {
		if otherServerID != "" {
			testPool.Exec(ctx, `DELETE FROM server WHERE id = $1`, otherServerID)
		}
		testPool.Exec(ctx, `DELETE FROM workspace WHERE slug = $1`, otherWorkspaceSlug)
	})

	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/workspaces", map[string]string{
		"name": "Handler Ops Other Workspace",
	...
	var createdWorkspace WorkspaceResponse
	if err := json.NewDecoder(w.Body).Decode(&createdWorkspace); err != nil {
		t.Fatalf("CreateWorkspace: decode response: %v", err)
	}
	otherWorkspaceID = createdWorkspace.ID

	w = httptest.NewRecorder()
	req = newRequest("POST", "/api/servers?workspace_id="+otherWorkspaceID, map[string]any{
		"name":           "foreign-node-01",
		"host":           "10.0.0.202",
		"environment":    "prod",
		"ssh_username":   "root",
		"credential_ref": "cred:test:foreign-node-01",
	})
	...
	var createdServer ServerResponse
	if err := json.NewDecoder(w.Body).Decode(&createdServer); err != nil {
		t.Fatalf("CreateServer: decode response: %v", err)
	}
	otherServerID = createdServer.ID

	w = httptest.NewRecorder()
	req = newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":            "Should not bind foreign server",
		"description":      "This must be rejected.",
		"target_server_id": otherServerID,
		"ops_intent":       "patch",
	})
	testHandler.CreateIssue(w, req)
	...
	if !strings.Contains(w.Body.String(), "target server not found in this workspace") {
		t.Fatalf("CreateIssue: expected cross-workspace error, got %s", w.Body.String())
	}
}
