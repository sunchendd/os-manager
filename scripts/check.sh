...
EXIT_CODE=0
ACTIVE_FRONTEND_PORT="$FRONTEND_PORT"
...
if curl -sf "http://localhost:${FRONTEND_PORT}" > /dev/null 2>&1; then
  ...
  if port_in_use "$FRONTEND_PORT"; then
    ACTIVE_FRONTEND_PORT=$(find_available_port $((FRONTEND_PORT + 1)))
    echo "    Frontend port :$FRONTEND_PORT is occupied by an unhealthy process; using :$ACTIVE_FRONTEND_PORT for checks"
  fi
  export FRONTEND_PORT="$ACTIVE_FRONTEND_PORT"
  export PLAYWRIGHT_BASE_URL="http://localhost:${ACTIVE_FRONTEND_PORT}"
  echo "    Building frontend for E2E..."
  pnpm --filter @multica/web build > /tmp/multica-check-frontend-build.log 2>&1 || {
    cat /tmp/multica-check-frontend-build.log
    EXIT_CODE=1
    exit 1
  }
  echo "    Starting frontend on :$ACTIVE_FRONTEND_PORT..."
  (
    cd apps/web &&
    pnpm exec next start --port "$ACTIVE_FRONTEND_PORT"
  ) > /tmp/multica-check-frontend.log 2>&1 &
  FRONTEND_PID=$!
  STARTED_FRONTEND=true
  sleep 2
  if ! kill -0 "$FRONTEND_PID" 2>/dev/null; then
    cat /tmp/multica-check-frontend.log
    EXIT_CODE=1
    exit 1
  fi
  wait_for_port "$ACTIVE_FRONTEND_PORT" "Frontend" 120 "/"
fi
